...
export function reviewerSystemPrompt(): string {
    return [
        ...
        "If diagnostics.expectedEmpty is true, score the rejection as excellent unless the raw output breaks JSON or includes reasoning.",
...
function descriptionSystemPrompt(): string {
    return [
        ...
        "For bullet mode only, ignore the bullet marker when matching context and return only the missing words after the current bullet text; for localLine.before '- 补充构建流程的', insert '关键步骤和验证方式', not '- 补充构建流程的'.",
